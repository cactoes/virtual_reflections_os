...
constexpr u64 bitmap_get_size(size_t size) {
    return size * (sizeof(u64) * 8);
}

template <size_t size>
constexpr u64 bitmap_get_size(u64 (&bitmap)[size]) {
...
template <size_t size>
bool bitmap_get(u64 (&bitmap)[size], size_t index) {
...
template <size_t size>
void bitmap_set(u64 (&bitmap)[size], size_t index, bool state) {
