...
#define STATIC_KSTRING(str)     (kstring_t){ (i8*)str, (u32)(sizeof(str) - 1), false }
...
static inline u64 __strlen(const char* cstr) {
...
struct kstring_t {
    ...
    kstring_t(const char* cstr) : length(__strlen(cstr)), buffer((i8*)cstr), is_heap(false) {};
...
kstring_t kstring_substring(const kstring_t* kstr, u64 pos, u64 count = KSTRING_MAX_LEN) {
    ...
    if (pos + count >= kstr->length || count == KSTRING_MAX_LEN)
...
void kstring_append(kstring_t* kstr, const char* cstr) {
    return kstring_append_simple(kstr, (const i8*)cstr, __strlen(cstr));
}
