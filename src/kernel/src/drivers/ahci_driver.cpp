        ...
        if (ports_implemented & (1 << i)) {
            ...
            switch (port->sig) {
                ...
                case AHCI_PORT_SIG_ATAPI: {
                    ...
                    if (!(drive.clb = ahci_atapi_port_init((hba_port_t*)port))) {
                        debug_print("ATAPI device failed to init port\n");
                        break;
                    }

                    if (const auto errc = ahci_atapi_identify_device(&drive); errc != 0) {
                        debug_print("ATAPI device failed to identify port: %i\n", errc);
                        break;
                    }
...
int ahci_atapi_prepare_command(ahci_cmd_context_t* ctx, ahci_sata_drive_t* drive, const uint8_t* atapi_packet, size_t packet_size, uint16_t data_length, bool write, uint8_t slot) {
    ctx->slot = slot;
