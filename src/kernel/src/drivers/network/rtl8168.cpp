...
void rtl8168_write_reg(rtl81xx_t* device, u32 offset, u32 value) {
    *((volatile u32*)((u8*)device->mmio_region + offset)) = value;
}

u32 rtl8168_read_reg(rtl81xx_t* device, u32 offset) {
    return *((volatile u32*)((u8*)device->mmio_region + offset));
}

bool rtl8168_load_mac(rtl81xx_t* device) {
    if (!device)
        return false;

    u32 mac_low = rtl8168_read_reg(device, RTL8168_MAC);
    u32 mac_high = rtl8168_read_reg(device, RTL8168_MAC + sizeof(u32));
...
bool rtl8168_init_device(const pci_device_t* pcie_device, rtl81xx_t* network_device) {
    ...
    if (pci_read_bar(pcie_device, 1) & PCI_BAR_MMIO_ENABLED)
        return false;
...
bool is_rtl8168_device(const pci_device_t* device) {
    return (device->vendor_device_id.vendor_id  == 0x10ec &&
           ...
           ||
           (device->vendor_device_id.vendor_id  == 0x10ec &&
           device->vendor_device_id.device_id   == 0x8139 &&
           device->class_info.class_code        == 0x2 &&
           device->class_info.sub_class         == 0x0);
}