...
bool subsys_dhcp_client_driver_t::init() {
    ...
    return nidm_udp_bind(get_global_nidm(), DHCP_PORT_CLIENT, { this, &subsys_dhcp_client_driver_t::network_callback }) == 0;
}

void subsys_dhcp_client_driver_t::shutdown() {
...
void subsys_dhcp_client_driver_t::network_callback(uint8_t* packet, size_t size) {
    // mutex_lock_guard guard(&mutex);
...
void subsys_dhcp_client_driver_t::configure(network_interface_device_t* device) {
    mutex_lock_guard guard(&mutex);
